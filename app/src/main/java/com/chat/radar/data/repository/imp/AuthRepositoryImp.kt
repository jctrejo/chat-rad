...
import com.chat.radar.util.SharedPrefConstants
...
class AuthRepositoryImp(
    ...
    override fun forgotPassword(email: String, result: (UiState<String>) -> Unit) {
        auth.sendPasswordResetEmail(email)
            .addOnCompleteListener { task ->
                ...
                result.invoke(UiState.Failure("Autenticación fallida, revisar correo electrónico"))
            }
    }
    ...
    override fun registerIngredients(
        ingredients: IngredientsArrayItem,
        user: User,
        result: (UiState<String>) -> Unit
    ) {
        val document = database.collection(FireStoreCollection.INGREDIENTS).document(user.id)
        user.id = document.id
        document
            .set(ingredients)
            .addOnSuccessListener {
    ...
    override fun registerSize(
        size: SizeArrayItem,
        user: User,
        keySize: String,
        result: (UiState<String>) -> Unit
    ) {
        val document = database.collection(FireStoreCollection.SIZE_PRODUCTS).document(keySize)
        document
            .set(size)
            .addOnSuccessListener {
                result.invoke(
